...
def load_and_process(filepath, req_col_name, skip_footer=0):
    ...
    if 'month_start' not in df.columns or req_col_name not in df.columns:
        print(f"Error: CSV {filepath} must contain 'month_start' and '{req_col_name}' columns.")
        return None
...
df_combined['requests_interactive'] = df_combined['requests_interactive'].fillna(0).astype(int)
df_combined['requests_batch'] = df_combined['requests_batch'].fillna(0).astype(int)
df_combined['total_requests'] = df_combined['requests_interactive'] + df_combined['requests_batch']
...
rects1 = ax.bar(x - bar_width/2, df_combined_sorted['requests_interactive'], bar_width, label='Interactive', color='#1f77b4')
rects2 = ax.bar(x + bar_width/2, df_combined_sorted['requests_batch'], bar_width, label='Batch', color='#ff7f0e')
...
ax.set_ylabel("Number of Inference Requests", fontsize=plot_axis_label_fs) # General Y-label
ax.set_title("Monthly Inference Requests (Interactive vs. Batch)", fontsize=plot_title_fs) # Updated title
ax.set_xticks(x)
