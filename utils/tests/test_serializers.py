...
class UtilsSerializersTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        with open(f"{base_path}/valid_chat_openai.json") as json_file:
            self.valid_chat_openai_params = json.load(json_file)
        ...
        with open(f"{base_path}/invalid_chat_openai.json") as json_file:
            self.invalid_chat_openai_params = json.load(json_file)
    ...
    # Test OpenAIParamSerializer for validation
    def test_OpenAIParamSerializer_validation(self):

        # For each valid set of parameters ...
        for valid_params in self.valid_chat_openai_params:

            # Send the data to the serializer and make sure the data is valid
            serializer = OpenAIParamSerializer(data=valid_params)
            self.assertTrue(serializer.is_valid(raise_exception=True))

        # For each invalid set of parameters ...
        for invalid_params in self.invalid_chat_openai_params:

            # Send the data to the serializer
            serializer = OpenAIParamSerializer(data=invalid_params)
