...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not redis_client:
            self.stdout.write(
                self.style.ERROR('Redis client not available. Using Django cache.clear()')
            )
            if not dry_run:
                cache.clear()
                self.stdout.write(self.style.SUCCESS('Cleared all Django cache'))
            else:
                self.stdout.write(self.style.WARNING('[DRY RUN] Would clear all Django cache'))
            return
        ...
        if clear_all:
            ...
            if not dry_run:
                ...
                self.stdout.write(
                    self.style.WARNING('[DRY RUN] Would clear ALL cache including Django sessions')
                )
            return
        ...
        for pattern in patterns:
            ...
            if keys:
                ...
                if not dry_run:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f'  Deleted {deleted} keys')
                    )
                else:
                    self.stdout.write(
